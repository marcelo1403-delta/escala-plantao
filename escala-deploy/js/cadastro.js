...
(function () {
  ...
  function updateLocks() {
    ...
    const canSec3 = hasId && hasPlantao;
  ...
  function turnoIcones(nome, alocacoes) {
    const n = norm(nome);
    const sol = '<span class="turno-icon icon-sol">&#9728;</span>';
    const lua = '<span class="turno-icon icon-lua">&#9790;</span>';
    return {
      d: alocacoes.diurno.has(n) ? sol : "",
      n: alocacoes.noturno.has(n) ? lua : ""
    };
  }
  ...
  function efetivoRowHtml(row, index, alocacoes) {
    const icones = turnoIcones(row.nome, alocacoes);
    ...
    const draggable = row.sit === "pres" && !alocarDisabled ? ' draggable="true"' : "";
    return `<tr class="${situacaoClass}${tipoClass}${alocarClass}" data-id="${esc(row.id)}" data-nome="${esc(row.nome)}" data-forca="${esc(row.forca)}" data-sit="${esc(row.sit)}" data-tipo="${esc(row.tipo)}"${draggable}>
      ...
      <td class="su">${icones.d}</td>
      <td class="mo">${icones.n}</td>
  ...
  function popularMotivosPopover(select, config = {}) {
    ...
    select.innerHTML = ["", "ATESTADO", "FERIAS", "LICENCA", "MISSAO", "OUTROS", "PERMUTA"]
  ...
  function servidoresElegiveisExtra() {
    ...
    return servidores.filter((s) => {
      ...
      if (s.plantao && norm(s.plantao) === plantao) return false;
  ...
  function popularNomeExtra(select, valorAtual = "") {
    ...
    select.options[0].disabled = true;
    select.options[0].hidden = true;
    servidoresElegiveisExtra().forEach((s) => {
  ...
  function criarLinhaNomeExtra(valor = "", removivel = true) {
    ...
    row.className = "s01-pop-row extra-name-row";
    row.innerHTML = `
      ...
      ${removivel ? '<button class="extra-remove-name" type="button">×</button>' : ""}
    `;
  ...
  function setupEfetivo() {
    ...
    ["popoverEfetivoPres", "popoverEfetivoAus", "popoverEfetivoBulk", "popoverIncluirPresencaExtra"].forEach((id) => {
      const pop = $(id);
